...
public class M2Dialog extends AlertDialog {
    ...
    public M2Dialog(Context context) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        View view = LayoutInflater.from(mContext).inflate(R.layout.imageview, null);
        Glide.with(mContext).load(Uri.parse(mUrl)).into((ImageView) view.findViewById(R.id.image_view));
        setContentView(view);

        Window window = getWindow();
        WindowManager.LayoutParams wl = window.getAttributes();
        int sw = App.mSP_Config.getInt(Constant.SCREEN_WIDTH, 0);
        wl.height = (sw * 400) / 640;
        wl.width = sw;
